...
class Trade:
    ...
    def get_first_set_of_closes(self) -> None:
        for kline in self.client.get_historical_klines(v_symbol, Client.KLINE_INTERVAL_1MINUTE, "1 hour ago UTC"):
            self.closes.append(float(kline[4]))

    def start(self) -> None:
        self.get_first_set_of_closes()
        self.twm.start()
        self.twm.start_kline_socket(callback=self.handle_socket_message,symbol=v_symbol, interval=Client.KLINE_INTERVAL_1MINUTE)

    def get_round_step_quantity(self, qty):
        info = self.client.get_symbol_info(v_symbol)
...
# Start The Trade
twm = ThreadedWebsocketManager(api_key=API_Config.API_KEY, api_secret=API_Config.API_KEY)
client = Client(API_Config.API_KEY, API_Config.API_KEY)
