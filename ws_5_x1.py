...
def whale_order_full(v_symbol, v_limit, v_son_fiyat, v_genel_orderbook, v_open_pri):
    ...
    v_volume_fark_oran = 0.001  # İlgili bid veya ask satırının tüm tablodaki volume oranı
    ...
    if v_alim_var == 0:
        ...
        try:
            ...
            if float(v_vol_oran_bid_old) >= 90 : #or float(v_bidask_fark_tutar_old*v_son_fiyat) >=10000:
                #v_1m_c = check_change_dk(v_symbol, '1m', 500)
                if 1==1 : #float(v_1m_c) > 0:
                    print('SEMBOL', v_symbol, '*PCTuttum Seni* HEDEF == ', "{:.6f}".format(float(v_hedef_bid)),
                          'Fark Tutar = ', "{:.1f}".format(float(v_bidask_fark_tutar)), 'Zaman = ', str(datetime.now())[0:19])

                    v_mess = str(v_symbol) + '--' + '*PC_Tuttum Seni* HEDEF == ' + '--' + "{:.6f}".format(float(v_hedef_bid)) + \
                             '--' + ' Zaman = ' + '--' + str(datetime.now())[0:19] + '--' + \
                             'Fiyat = ' + '--' + "{:.6f}".format(float(v_son_fiyat)) + '--' + \
        ...
        try:
            ...
            if float(v_son_fiyat) >= float(v_hedef_bid_global):
                ...
                v_mess1 = 'PC_Karla Sattı..Hedefi : ' + "{:.6f}".format(float(v_hedef_bid_global)) + '- Sembol :' + str(
                    v_symbol) + \
                ...
                v_mess1 = 'PC_Zararla Sattı..Hedefi : ' + "{:.6f}".format(float(v_hedef_ask_global)) + '- Sembol :' + str(
                    v_symbol) + \
                          '- Alım Fiyatı :' + "{:.6f}".format(
                    float(v_alim_fiyati)) + ' - Satım Fiyatı : ' + "{:.6f}".format(float(v_son_fiyat)) + \
    ...
    try:
        while (True):
            if v_alim_var == 0:
               ...
               # v_genel_orderbook = get_snapshot(v_sembol_g, v_limit_g)
                print('2...', len(v_genel_orderbook["bids"]))
                time.sleep(1)
