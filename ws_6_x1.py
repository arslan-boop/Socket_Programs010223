...
def whale_order_full(v_symbol, v_limit, v_son_fiyat, v_genel_orderbook, v_open_pri, v_islem_tutar, v_volume_fark_oran,
    ...
    if v_alim_var == 0:
        ...
        try:
            ...
            if  v_bidask_fark_tutar >= 0 and v_bidask_fark_tutar_old >= 0 and \
                float(v_vol_oran_bid) >= float(v_volume_fark_oran * 100) and \
                float(v_vol_oran_bid)> float(v_vol_oran_ask)+3  and \
                float(v_vol_oran_bid_old) > 45:
                # ************************************Alım İşlemi******************************************
                if v_test_prod == 'P':
