...
def on_message_f(ws_front, message):
    ...
    print('ON-FRONT message SEMBOL=', str(json_message['s']), 'Son fiyat  = ', v_last_price_g)
...
# *******************************************************Order Book Socketi
def on_open_o(ws_order):
    print('opened connection', vn_order)
    # subscribe_message = {"method": "SUBSCRIBE", "params": vn_order, "id": 1}
    # ws_order.send(json.dumps(subscribe_message))


def on_error_o(ws_order):
    print('Error olustu')

def on_close_o(ws_order):
    print('closed connection')


def on_message_o(ws_order, message):
    global v_last_price_g
    json_message = json.loads(message)
    # print('Gelen mesaj: ', json_message)
    candle = json_message['k']
    v_last_price_g = candle['c']
    print('ON- message SEMBOL=', str(json_message['s']), 'Son fiyat  = ', v_last_price_g)


# 1 saniyelik stream verilerle kline daki son fiyatı vs alır
def socket_orderbook(v_symbol, v_inter):
    global vn_order, ws_order
    v_symbol = v_symbol.lower()
    # v_sembol_deg1 = f'[{v_symbol}@depth]'
    # v_sembol_deg5 = v_sembol_deg1.replace("[", "")
    # v_sembol_deg5 = v_sembol_deg5.replace("]", "")
    vn_order = 'dene'#[v_sembol_deg5]
    #socket_o = 'wss://stream.binance.com:9443/ws'
    socket_o = "wss://stream.binance.com:9443/ws/" + v_symbol + "@depth",
    #wss://stream.binance.com:9443/ws/steembtc@depth",
    #ws_front = websocket.WebSocketApp(socket_f, on_message=on_message_f, on_open=on_open_f, on_close=on_close_f, on_error=on_error_f)
    ws_order = websocket.WebSocketApp(socket_o, on_message=on_message_o, on_open=on_open_o, on_close=on_close_o)
...
def run_order_book(v_sembol_g, v_inter_g, v_limit_g, v_in_g):
    while (True):
        try:
            socket_orderbook(v_sembol_g, v_inter_g)
            time.sleep(1.666)
        #except:
        except Exception as exp:
            v_hata_mesaj = 'Hata Oluştu!!..T1  = ' + str(exp)
...
#     except Exception as exp:
#             v_hata_mesaj = 'Hata Oluştu!!..run_order_book_ozet  = ' + str(exp)
#
# def run_order_book(v_sembol_g, v_inter_g, v_limit_g, v_in_g):
#     while (True):
#         try:
...
def hesapla_full(v_sembol_g, v_limit_g, spot_client, v_last_price_g):
    while (True):
        try:
            v_genel_orderbook =ws_3_order7_s.get_ordergenelorder_book()
...
# def ana_islem(v_sembol_g, v_limit_g, spot_client, v_last_price_g, v_inter_g, v_in_g):
#     # Coini sürekli dinleyen socket stream
#     t1 = threading.Thread(target=run_frontdata, args=(v_sembol_g, v_inter_g), daemon=True)
#     t2 = threading.Thread(target=run_order_book, args=(v_sembol_g, v_inter_g, v_limit_g, v_in_g), daemon=True)
#     t1.start()
#     time.sleep(1.666)
#     t2.start()
#     time.sleep(5)
...
if __name__ == '__main__':
    try:
        ...
        # Coini sürekli dinleyen socket stream --, daemon=True
        ...
        t2 = threading.Thread(target=ws_3_order7_s.basla, args=(v_limit_g, v_sembol_g))
        t2.start()
        # time.sleep(1.666)
        # time.sleep(5)
        #v_genel_orderbook = ws_3_order7_s.get_ordergenelorder_book()
        #print('v_cccccccccccccccccccccccccGenl book', v_genel_orderbook)
        #ws_3_order7_s.basla(v_limit_g, v_sembol_g)
        # t3 = threading.Thread(target=hesapla_full, args=(v_sembol_g, v_limit_g, spot_client, v_last_price_g))
        # t3.start()
        print('İşlenen Coin -------', v_sembol_g, 'Son Fiyat', v_last_price_g)
    except Exception as exp:
        v_hata_mesaj = 'Hata Oluştu!!..T1  = ' + str(exp)
