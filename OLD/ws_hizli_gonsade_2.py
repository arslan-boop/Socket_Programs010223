...
def on_message_f(ws_front, message):
    global v_last_price_g, v_open_price, closes, highes, lowes, openes, v_kesim, \
        v_ters_kesim, v_alim_var, v_hizli_gonzales, v_ziplama_oran, kesmeler, v_alim_zamani
    ...
    if is_candle_closed:  # or (v_acil_al == 1 and is_candle_closed == True):
        candle_islem(v_symbol, kesmeler, open, close, high, low, closes, highes, lowes, openes, v_ziplama_oran,v_zaman)

    # ************************Acil alım
    if v_acil_al == 1 and is_candle_closed == False:
        if v_alim_var == 0 and v_hizli_gonzales != 1:
            candle_islem_acil_alim(v_symbol, kesmeler, open, close, high, low, closes, highes, lowes, openes, v_ziplama_oran,v_zaman)
...
# *********************************************************************************************
def candle_acil_satim(v_symbol, open, close, high, low, closes, highes, lowes, openes, v_ziplama_oran):
...
# **********************************************************************************************************************
def candle_islem(v_symbol, kesmeler, open, close, high, low, closes, highes, lowes, openes, v_ziplama_oran,v_zaman):
    global v_last_price_g, v_open_price, v_kesim, v_ters_kesim, v_alim_var, v_hizli_gonzales
    v_toplam = 0
    try:
        closes.append(close)
        ...
        if v_alim_var == 0:
            ...
            v_girme, v_1m_c = check_full_kontrol(v_symbol, openes, closes, highes, lowes, v_zaman)
...
# *********************************************
def candle_islem_acil_alim(v_symbol, kesmeler, open, close, high, low, closes, highes, lowes, openes, v_ziplama_oran,v_zaman):
    global v_last_price_g, v_open_price, v_kesim, v_ters_kesim, v_alim_var, v_hizli_gonzales
    v_toplam = 0
    try:
        # time.sleep(2.333)
        ...
        v_ema_cross_up, v_ema_cross_down, ema_artik = check_exist_ema_second(v_symbol, openes, closes, highes, lowes,10, 30)
        v_girme, v_1m_c = check_full_kontrol(v_symbol, open, close, high, low, v_zaman)
...
# ************************
def check_full_kontrol(v_symbol, openes, closes, highes, lowes, v_zaman_dk):
    v_girme = 0
    v_uz = len(closes)
    v_len = len(closes)
    v_l_c_p_c = closes[-1]
    ...
    for i in range(1,v_say):
        ...
        if float(closes[-i-1]) > float(v_l_c_p_c):
            v_girme =   v_girme + 1

    return v_girme,v_l_c_p
