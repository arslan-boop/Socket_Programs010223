...
def whale_order_full(v_symbol, v_limit, v_son_fiyat, v_genel_orderbook, v_open_pri,v_islem_tutar):
    ...
    if v_alim_var == 0:
        ...
        try:
            ...
            if v_bid_len > 0 and v_bidask_fark_tutar >= 0 and float(v_vol_oran_bid) >= float(v_volume_fark_oran * 100):  # and v_vol_oran_ask <1:
                #************************************Alım İşlemi******************************************
                ...
                v_mess = str(v_symbol) + '--' + '*Tuttum Seni* HEDEF == ' + '--' + "{:.6f}".format(
                    float(v_hedef_bid)) + \
                         ...
                         'Miktar = ' + '--' + "{:.6f}".format(float(v_alim_miktar)) + '--' + \
                         'İşlem Tutar = ' + '--' + "{:.6f}".format(float(v_islem_tutar)) + '--' + \
        ...
        try:
            ...
            if float(v_son_fiyat) >= float(v_hedef_bid_global):
                ...
                # ***************************SATIM*******************************************
                order_sell = v_client.order_market_sell(symbol=v_symbol, quantity=v_alim_miktar)
                # ***************************************************************************
                v_son_fiyat = float(v_last_price_g)
...
def dosya_aktar():
    ...
    with open('../Sembol3.txt', 'r') as dosya:
        ...
        for line in dosya.read().splitlines():
            ...
            if 1 == 1:  # v_3m_c > 0 and v_5m_c :# and v_15m_c > 0 and v_60m_c > 0:
                if i <= 9:
    ...
    try:
        while (True):
            ...
            if v_kota_doldu >=2:
                ...
                if v_alim_var == 1:
                    time.sleep(0.01)
                    whale_order_full(v_sembol_g, v_limit_g, float(v_last_price_g), v_genel_orderbook, v_open_price,v_islem_tutar)
            else:
                if v_alim_var == 0:
                    v_genel_orderbook = orderbook  # get_snapshot(v_sembol_g, v_limit_g)
                    time.sleep(0.01)
...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            v_islem_tutar = 100 #float(usdtBalance) * 0.4
            ...
            while True:
                ...
                with concurrent.futures.ProcessPoolExecutor(max_workers=v_maxw) as executer:
                    results = [executer.submit(main_islem, v_dosya_coin[p], v_limit_g, v_inter_g,v_islem_tutar) for p in
