...
if __name__ == '__main__':
    try:
        ...
        # t2 = threading.Thread(target=ws_3_order8_s.basla, args=(v_limit_g, v_sembol_g))
        # t2.start()
        # time.sleep(5)
        ...
        # Emir defterinin snapshotını alıp sürekli güncelliyor.
        # t4 = threading.Thread(target=ws_3_order8_s.get_ordergenelorder_book())
        # t4.start()
        #time.sleep()
        while True:
            v_genel_orderbook = ws_3_order8_s.get_ordergenelorder_book()
            print('Order  book', v_genel_orderbook)
            # print('Order Dizi Bids =', len(v_genel_orderbook["bids"]))
            # print('Order Dizi Asks=', len(v_genel_orderbook["asks"]))
