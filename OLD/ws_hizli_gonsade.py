...
def on_message_f(ws_front, message):
    ...
    if x> v_ziplama_oran and v_alim_var==0:
        v_acil_al = 1
        v_acil_oran = x
    else:
        v_acil_al = 0
    ...
    if is_candle_closed or (v_acil_al == 1 and v_alim_var==0):
        if is_candle_closed:
            v_mum_kapandi = 1
        else:
            v_mum_kapandi = 0
        # print(close, 'closes dizisi çerde', v_symbol, closes, datetime.now()),
        candle_islem(v_symbol, v_acil_al, v_acil_oran, kesmeler, open, close, high, low, closes, highes, lowes, openes,
                     v_ziplama_oran,v_mum_kapandi)


# **********************************************************************************************************************
def candle_islem(v_symbol, v_acil_al, v_acil_oran, kesmeler, open, close, high, low, closes, highes, lowes, openes,
                 v_ziplama_oran,v_mum_kapandi):
    ...
    if v_alim_var ==0:
        ...
        ema_artik, v_10m_c = check_exist_ema_second(v_symbol, openes, closes, highes, lowes, 10, 30)
        # # v_ziplama_oran = 0.5
        # if v_acil_al == 1:
        #     v_1m_c = v_acil_oran
        # else:
        #     v_1m_c = v_1m_c
        # Geçmişe yönelik kesimleri tutup son 5 dk vs içinde kesme varsa gir demek için
        #if v_acil_al == 0:
        if v_ema_cross_up == 1:
           kesmeler.append(1)
        else:
           kesmeler.append(0)
                # print('Kesme', kesmeler)
        ...
        for i in range(len(kesmeler)):
            # print('dasd',kesmeler[-i-1])
            v_toplam = v_toplam + int(kesmeler[-i - 1])

        # print('Toplam', v_toplam)
        if v_1m_c > 30:
        ...
        if v_1m_c > v_ziplama_oran and close > float(v_3m_c) and close > float(v_5m_c) and (v_toplam >= 1 or ema_artik == 1) and \
                close > float(v_10m_c) and close > float(v_15m_c):  # and close > float(v_30m_c) and close > float(v_45m_c) and close > float(v_60m_c):
            # EKSTRA KONTROL
        ...
        # Acil alım nedeniyle mum kapanmadan eklenenleri düzeni bozmasın diye çıkarıyoruz.Alım olsun yada olmasın
        if  v_acil_al==1 and v_mum_kapandi == 0:
            closes.pop(-1)
            highes.pop(-1)
            lowes.pop(-1)
            openes.pop(-1)
        ...
        #Alım var ve mum kapandığında normal şartlarda geldiyse kapanış süreci işleyecek
        if v_mum_kapandi==1:
            if v_ema_cross_down == 1 or float(openes[-1]) > float(closes[-1]):
                v_ters_kesim = 1
...
def check_exist_ema_second(v_symbol, open, close, high, low, v_kisa, v_uzun):
    ...
    if v_uz < 2:
        ...
        # ********************************************
        if v_len < 2:
            return ema_cross_upk, ema_cross_downk, 0, 0, 0, 0, 0, 0, 0, ema_artik
        else:
            v_p_c_p2 = close[-2]
        ...
        if v_len < 4:
            return ema_cross_upk, ema_cross_downk, v_1m_c, 0, 0, 0, 0, 0, 0, ema_artik
        else:
            v_p_c_p3 = close[-3]
            # v_3m_c = float(((v_l_c_p - v_p_c_p3) * 100) / v_p_c_p3)
        if v_len < 6:
            # return ema_cross_upk, ema_cross_downk, v_1m_c, v_3m_c, 0, 0, 0,0,0
            return ema_cross_upk, ema_cross_downk, v_1m_c, v_p_c_p3, 0, 0, 0, 0, 0, ema_artik
        else:
            v_p_c_p5 = close[-5]
        ...
        if v_len < 11:
            ...
            return ema_cross_upk, ema_cross_downk, v_1m_c, v_p_c_p3, v_p_c_p5, 0, 0, 0, 0, ema_artik
        else:
            v_p_c_p10 = close[-10]
        ...
        if v_len < 31:
            ...
            return ema_cross_upk, ema_cross_downk, v_1m_c, v_p_c_p3, v_p_c_p5, v_p_c_p15, 0, 0, 0, ema_artik
        else:
            v_p_c_p30 = close[-30]
        ...
        if v_len < 46:
            ...
            return ema_cross_upk, ema_cross_downk, v_1m_c, v_p_c_p3, v_p_c_p5, v_p_c_p15, v_p_c_p30, 0, 0, ema_artik
        else:
            v_p_c_p45 = close[-45]
        ...
        if v_len < 61:
            ...
            return ema_cross_upk, ema_cross_downk, v_1m_c, v_p_c_p3, v_p_c_p5, v_p_c_p15, v_p_c_p30, v_p_c_p45, 0, ema_artik
        else:
            v_p_c_p60 = close[-60]
        ...
        return ema_cross_upk, ema_cross_downk, v_1m_c, v_p_c_p3, v_p_c_p5, v_p_c_p15, \
               v_p_c_p30, v_p_c_p45, v_p_c_p60, ema_artik, v_p_c_p10
...
def get_first_set_of_closes(v_symbol, v_inter):
    ...
    try:
        if v_inter == '1m':
            for kline in v_client.get_historical_klines(v_symbol, '1m', "5 hour ago UTC"):
