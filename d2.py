...
def whale_order_full(v_symbol, v_limit, v_son_fiyat, v_genel_orderbook, v_open_pri):
    ...
    if v_alim_var == 0:
        ...
        try:
            ...
            # Aşağıda son fiyata göre bir oranlama var. Burada ise son fiyatın altı veya üstünden bağımsız
            #Belirlenen aralıktan bis ve asklerin hacimleri ve oranları çıkarılıyor
            v_bidask_fark_tutar_old = float(bid_tbl['volume'].sum()) - float(ask_tbl['volume'].sum())
            v_vol_oran_bid_old = (float(bid_tbl['volume'].sum()) / float(volumewhale)) * 100
            v_vol_oran_ask_old = (float(ask_tbl['volume'].sum()) / float(volumewhale)) * 100
            ...
            if v_bid_len > 0 and v_bidask_fark_tutar >= 0 and float(v_vol_oran_bid) >= float(v_volume_fark_oran*100):
                ...
                if 1==1 : #float(v_1m_c) > 0:
                    ...
                    v_mess = str(v_symbol) + '--' + '*Tuttum Seni* HEDEF == ' + '--' + "{:.6f}".format(float(v_hedef_bid)) + \
                             '--' + ' Zaman = ' + '--' + str(datetime.now())[0:19] + '--' + \
                             'Fiyat = ' + '--' + "{:.6f}".format(float(v_son_fiyat)) + '--' + \
    ...
    try:
        while (True):
            if v_alim_var == 0:
                ...
                v_genel_orderbook = get_snapshot(v_sembol_g, v_limit_g)
                #print('2...', v_sembol_g, datetime.now())
            else:
