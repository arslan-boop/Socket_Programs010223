...
def on_message_f(ws_front, message):
    global v_last_price_g, v_open_price, closes, highes, lowes, \
        v_kesim, v_ters_kesim, v_alim_var, v_hizli_gonzales,v_dosya_coin
    ...
    print('closes dizisi ik', v_symbol, len(highes), datetime.now())
    # print('Geldi', v_symbol, close, '-', open, '-', high, '-', low, '-', datetime.now())
    is_candle_closed = candle['x']

    if is_candle_closed:
        y = np.where(closes == v_symbol)
        v_index = int(y[0])
        #print('indec',v_index)
        #print(close, 'closes dizisi çerde', v_symbol, closes, datetime.now())
        time.sleep(0.1)
        #print(close, 'closes dizisi çerde', v_symbol, highes, datetime.now())
        closes = numpy.append(closes[v_index], close)
        highes = numpy.append(highes[v_index], high)
        lowes = numpy.append(lowes[v_index], low)
        #print(close, 'closes dizisi çerde', v_symbol, closes, datetime.now())
        #closes.append(close)
        # highes.append(high)
        # lowes.append(low)
        print('Geldi--kapa', v_symbol, closes, datetime.now())
    ...
    if v_ema_cross_down3m == True:
        ...
        v_ters_kesim = 0
    """
    # print('Boyu', len(closes[0]))
    # print('Boyu', len(closes))
    if len(closes[0]) > 303:
        for i in range(0,len(v_dosya_coin)):
            closes[i].pop(1)
            highes[i].pop(1)
            lowes[i].pop(1)
        print('Boyu', closes)
...
def main_islem(v_sembol_g, v_limit_g, v_inter_g, v_islem_tutar, v_volume_fark_oran, v_oran, v_kar_oran,
               v_zarar_oran, minVolumePerc, v_test_prod,v_closes,v_highes,v_lowes):
    global closes,highes,lowes
    print('Başladı ', v_sembol_g, datetime.now())
    #print('Sonuç', v_closes)
    closes = v_closes
    highes = v_highes
    lowes = v_lowes
    #print('Sonuç--222', closes)
    try:
        time.sleep(1.333)
...
# ****************************************************EMA ve V_Client **************************************************
def get_first_set_of_closes(v_inter):
    global closes, highes, lowes, v_dosya_coin
    list_close = []
    list_high = []
    list_low = []
    if v_inter == '1m':
        v_gecmis_zaman = "5 hour ago UTC"
        v_kac_adet = 301
    else:
        v_gecmis_zaman = "5 minute ago UTC"
        v_kac_adet = 301
    try:
        p = 0
        i = 1
        # Önce Tüm sembol isimlerini ve değerleri aynı diziye doldur. Sonra boyutları yeniden şekillendir.
        for p in range(len(v_dosya_coin)):
            v_symbol = v_dosya_coin[p]
            list_close.append(v_symbol)
            list_high.append(v_symbol)
            list_low.append(v_symbol)

            # print('Doldurdu', list1, datetime.now())
            for kline in v_client.get_historical_klines(v_symbol, v_inter, v_gecmis_zaman):
                # tüm değerleri stringe çevirip listeye stıyoruz
                list_close.append(str(float(kline[4])))
                list_high.append(str(float(kline[2])))
                list_low.append(str(float(kline[3])))

        # NP ile yeniden satır ve sütünları oluştur
        nd_arr_c = np.array(list_close)
        closes = nd_arr_c.reshape(len(v_dosya_coin), v_kac_adet)
        # x = np.where(nd_arr_c == 'LSKBUSD')
        # y = np.where(closes == 'LSKBUSD')
        nd_arr_h = np.array(list_high)
        highes = nd_arr_h.reshape(len(v_dosya_coin), v_kac_adet)
        nd_arr_l = np.array(list_low)
        lowes = nd_arr_l.reshape(len(v_dosya_coin), v_kac_adet)
        # closesss =numpy.append(closesss[0],'889999999999999999')
        #print('Sonuç', closes)
        # print('Sonuç', x,'Yesi=',y[0],'fds',y)
        #print('son')
    ...
    try:
        while True:
            ...
            while True:
                ...
                get_first_set_of_closes(v_inter_g)
                #print('Clos', highes)
                ...
                with concurrent.futures.ProcessPoolExecutor(max_workers=v_maxw) as executer:
                    results = [executer.submit(main_islem, v_dosya_coin[p], v_limit_g, v_inter_g, v_islem_tutar,
                                               ...
                                               v_zarar_oran, minVolumePerc, v_test_prod,closes,highes,lowes) for p
                               in
