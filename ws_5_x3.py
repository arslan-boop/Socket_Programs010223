    ...
    if v_alim_var == 0:
        ...
        try:
            ...
            if v_bid_len > 0 and v_bidask_fark_tutar >= 0 and float(v_vol_oran_bid) >= float(v_volume_fark_oran*100):
                ...
                if 1 == 1:  # float(v_1m_c) > 0:
                    ...
                    v_mess = str(v_symbol) + '--' + '*PC_Tuttum Seni* HEDEF == ' + '--' + "{:.6f}".format(
                        float(v_hedef_bid)) + \
                             '--' + ' Zaman = ' + '--' + str(datetime.now())[0:19] + '--' + \
                             'Fiyat = ' + '--' + "{:.6f}".format(float(v_son_fiyat)) + '--' + \
        ...
        try:
            ...
            if float(v_son_fiyat) >= float(v_hedef_bid_global):
                ...
                v_mess1 = 'PC_Karla Sattı..Hedefi : ' + "{:.6f}".format(float(v_hedef_bid_global)) + '- Sembol :' + str(v_symbol) + \
                ...
                v_mess1 = 'PC_Zararla Sattı..Hedefi : ' + "{:.6f}".format(float(v_hedef_ask_global)) + '- Sembol :' + str(v_symbol) + \
                          '- Alım Fiyatı :' + "{:.6f}".format(float(v_alim_fiyati)) + ' - Satım Fiyatı : ' + "{:.6f}".format(float(v_son_fiyat)) + \
                          '- Zarar Oranı :' + "{:.6f}".format(float(v_zarprofit_oran)) + ' - Zaman : ' + str(datetime.now())[0:19] + 'Alım Zamanı : ' + str(v_alim_zamani)
                print(v_mess1)
                ...
                if float(v_son_fiyat) > float(v_alim_fiyati):
                    ...
                    v_mess1 = vm_karzar + '...Hedefi : ' + "{:.6f}".format(float(v_hedef_bid_global)) + '- Sembol :' + str(v_symbol) + \
                              '- Alım Fiyatı :' + "{:.6f}".format(float(v_alim_fiyati)) + ' - Satım Fiyatı : ' + "{:.6f}".format(float(v_son_fiyat)) + \
                              '- Kar Oranı :' + "{:.6f}".format(float(v_profit_oran)) + ' - Zaman : ' + str(datetime.now())[0:19] + 'Alım Zamanı : ' + str(v_alim_zamani)
                    v_karzarar_mesaj = str(v_symbol) + '*' + 'Kar' + '*' + "{:.6f}".format(float(v_alim_fiyati)) + '*' + "{:.6f}".format(float(v_son_fiyat)) + \
                                       '*' + "{:.3f}".format(float(v_profit_oran)) + '*' + str(datetime.now())[0:19]
                    print(v_mess1)
                    # ******************
